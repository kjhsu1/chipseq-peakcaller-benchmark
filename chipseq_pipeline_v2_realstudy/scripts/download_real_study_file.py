...
def main() -> None:
    ...
    head_response = session.head(remote_url, allow_redirects=True, timeout=timeout)
    head_response.raise_for_status()
    remote_size_text = head_response.headers.get("Content-Length", "").strip()
    remote_size = int(remote_size_text) if remote_size_text.isdigit() else None
