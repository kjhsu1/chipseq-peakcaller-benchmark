...
class DepthStats:
    ...
    def __init__(
        ...
        self.cv_depth = cv_depth
        self.gini_depth = gini_depth
...
def optional_path(value: object, base_dir: Path | None = None) -> Path | None:
    """Return a resolved path or None for blank manifest values."""
    if value is None:
        return None
...
def file_ready(path: Path | None) -> bool:
    """Return whether a required local file exists."""
