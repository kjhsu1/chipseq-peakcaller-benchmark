...
def compute_row_metrics(row: pd.Series, half_window: int) -> dict:
    ...
    base_dir = Path(str(row.get("_manifest_dir", ".")))
    treatment_bam = optional_path(row.get("treatment_bam"), base_dir)
    control_bam = optional_path(row.get("control_bam"), base_dir)
    peak_bed = optional_path(row.get("peak_bed"), base_dir)
    background_bed = optional_path(row.get("background_bed"), base_dir)
    peak_intervals = load_bed_intervals(peak_bed)
    peak_depths = estimate_depths(treatment_bam, peak_intervals)
    peak_stats = summarize_depths(peak_depths)
    lengths = interval_lengths(peak_intervals)
    profile = mean_metaprofile(treatment_bam, peak_intervals, half_window)
    profile_stats = metaprofile_metrics(profile)
    control_ratio = 0.0
    control_stats = {"background_mean_depth": 0.0, "background_cv_depth": 0.0, "background_gini_depth": 0.0, "background_bumpiness": 0.0}
    if file_ready(background_bed):
        background_intervals = load_bed_intervals(background_bed)
        background_depths = estimate_depths(treatment_bam, background_intervals)
        background_summary = summarize_depths(background_depths)
        control_stats = {
            "background_mean_depth": background_summary.mean_depth,
            "background_cv_depth": background_summary.cv_depth,
            "background_gini_depth": background_summary.gini_depth,
            "background_bumpiness": background_summary.bumpiness,
        }
    elif file_ready(control_bam):
        background_depths = estimate_depths(control_bam, peak_intervals)
        background_summary = summarize_depths(background_depths)
        control_stats = {
            "background_mean_depth": background_summary.mean_depth,
            "background_cv_depth": background_summary.cv_depth,
    ...
    if file_ready(control_bam):
        control_depths = estimate_depths(control_bam, peak_intervals)
    ...
    return {
        ...
        "treatment_peak_median_depth": peak_stats.median_depth,
...
def main() -> None:
    ...
    manifest["_manifest_dir"] = str(args.input_manifest.parent.resolve())
    args.output_dir.mkdir(parents=True, exist_ok=True)

    if "background_bed" not in manifest.columns:
        manifest["background_bed"] = ""

    required = manifest.apply(
        lambda row: file_ready(optional_path(row.get("treatment_bam"), args.input_manifest.parent))
        and file_ready(optional_path(row.get("peak_bed"), args.input_manifest.parent)),
        axis=1,
    )
    ...
    skipped_rows = manifest.loc[~manifest.index.isin(available_rows.index)].copy()
    ...
    metric_names = [
        ...
        "background_cv_depth",
        "treatment_peak_gini",
        "background_gini_depth",
        "treatment_peak_bumpiness",
        "background_bumpiness",
        "summit_height",
        "half_max_width",
        "auc",
        "symmetry",
    ...
    sim_df.to_csv(args.output_dir / "simulated_run_metrics.csv", index=False)
    ref_df.to_csv(args.output_dir / "real_reference_metrics.csv", index=False)
    ...
    skipped_rows.to_csv(args.output_dir / "skipped_manifest_rows.csv", index=False)
